<div class="row">
    <div class="col-md-9">
        ...
        @component('components.box')
            @slot('type', $box_type)
            @slot('icon', 'users')
            @slot('title', 'Agentes da equipe')
            
            {{ Form::bsDuallistbox('agentes[]', $agentes, null, ['class' => 'agentes']) }}
        @endcomponent
    </div>
    <div class="col-md-3">
        @component('components.box')
...
<script>
    $(function () {
        $('.agentes').bootstrapDualListbox('setNonSelectedListLabel', 'Agentes')
            .bootstrapDualListbox('setSelectedListLabel', 'Agentes selecionados');
    });
</script>
@endpush
