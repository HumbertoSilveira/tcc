    ...
    @component('components.box')
        ...
        @slot('actions')
            ...
            @if(Gate::allows('cadastrar-'.str_slug($txt_plural)))
                <a href="{{ route('modulos.create') }}" class="btn btn-flat bg-olive btn-sm btn-toggle">
                    <i class="fa fa-plus fa-fw"></i> Adicionar {{ $txt_singular }}
                </a>
            @endif

            @if(Gate::allows('excluir-'.str_slug($txt_plural)))
                <a href="{{ route('modulos.destroy') }}" class="btn btn-flat bg-maroon btn-sm btn-delete-all">
                    <i class="fa fa-times fa-fw"></i> Excluir módulo
                </a>
            @endif
